...
# and some more things by Egidijus Ligeika
#

# Usage: zfs_stats e | cl | ct | u | lu | le
#
# Available functions:
#  e    = efficiency - ARC efficiency
#  cl   = cachehitlist - Cache hit by cache list
#  ct   = cachehitdtype - Cache hit by data type
#  u    = utilization - ARC size breakdown
#  lu   = l2utilization - L2ARC size breakdown
#  le   = l2efficiency - L2ARC efficiency
#
...
if ! type "zpool" > /dev/null; then
    cat << EOF
    #######################################################
    zpool command or binary not found, please install ZFS.
    #######################################################
EOF
fi
